...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback, GoogleApiClient.ConnectionCallbacks,
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        if(getIntent().hasExtra("name")) {
            nearbyLat = Double.parseDouble(getIntent().getStringExtra("latitude"));
            nearbyLong = Double.parseDouble(getIntent().getStringExtra("longitude"));
            nearbyName = getIntent().getStringExtra("name");
            //Add a marker in Sydney and move the camera
            LatLng nearby = new LatLng(nearbyLat, nearbyLong);
            mMap.addMarker(new MarkerOptions().position(nearby).title(nearbyName));
        }
    ...
    public void onLocationChanged(Location location) {
        ...
        mMap.animateCamera(CameraUpdateFactory.zoomBy(7f));
